...
/*--------------------------------------------------*/
/*  Submit Listings Update Form Admin
/*--------------------------------------------------*/
							...
							$.ajax({
								...
								success:function(data){
									...
									if(data.response === true){
										...
										window.location.href = baseUrl+'emailActive';
									}
									else{
...
$(document).on('submit','#UserRegistrationFormAdmin',function(e){
	e.preventDefault();
	var csrfName = $('.txt_csrfname').attr('name');
	var csrfHash = $('.txt_csrfname').val();
	var form = $(this);
...
/*--------------------------------------------------
|  Withdrawal Request Refferal
/*--------------------------------------------------*/
$(document).on('submit','#withdrawFormRefferal',function(e){
	e.preventDefault();	
	$('#loader').show();
	...
	$.ajax({ type: "POST", url: baseUrl  + "refferal/create_withdrawal", data: $("#withdrawFormRefferal").serialize(),dataType: 'json',cache: false,
		success:function(data){
			$('.txt_csrfname').val(data.token);
			if(data.response === true){
				$('#loader').hide();
...
/*--------------------------------------------------
|  Withdrawals Ref Filter Change
/*--------------------------------------------------*/
$(document).on('change', '#filter_type_withdraw_ref', function(e){  
	e.preventDefault();
	loadWithdrawalsDataRef($("#filter_type_withdraw_ref").val());
});

/*--------------------------------------------------
|  Refferal Withdrawal History Pagination 
/*--------------------------------------------------*/
$(document).on("click", ".paginationWithdrawalsRef li a", function(e){
	e.preventDefault();
	var csrfName = $('.txt_csrfname').attr('name');
	var csrfHash = $('.txt_csrfname').val();

	$.ajax({
		url:baseUrl+"refferal/user_withdrawals_ref/"+ $(this).attr('data-ci-pagination-page'),
		method:"POST",
		data:{[csrfName]: csrfHash},
		dataType: 'json',
		success:function(data){
