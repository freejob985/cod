...
function getListingsData(data){
    ...
    $('td', row).eq(2).html("<a href='"+baseUrl+'admin/edit_listings/'+data.listing_type+'/'+data.id+"' title='Click to Edit Listing'>"+data.website_BusinessName+"</a>");
    if(data.status === '1'){
...
function getListingHeaderData(data){
    ...
    $('#tbl_ListingsData tbody').off( 'click', '.editLink' ).on( 'click', '.editLink', function (e) {
        ...
        if (typeof(data_row) != "undefined"){        
            ...
            $('#listing_description').summernote('destroy');
            $('#listing_description').val(data.response[0].listing_description).summernote();
...
function getUserControlData(data){
    ...
    },
    {
        "data": null,
        "className": 'center',
        "defaultContent": '<a href="" class="editUser"><i title="delete" class="fa fa-pencil" aria-hidden="true"></a>'
    },
    {
        "data": null,
        "className": 'center',
        "defaultContent": '<a href="" class="deleteUser"><i title="delete" class="fa fa-trash" aria-hidden="true"></a>'
    }
    ],
    "order": [[1, 'asc']],
    dom: 'Bfrtip',
    ...
    if(typeof(data.user_status) !== "undefined"){
        if (data.user_id =='1') {
            $('td', row).eq(8).html("<a href='' class=''></a>");
            $('td', row).eq(9).html("<a href='' class=''></a>");
        }
    ...
    $('#tbl_userdata tbody').off( 'click', '.deleteUser' ).on( 'click', '.deleteUser', function (e) {
        e.preventDefault();
        ...
        if (typeof(data_row) != "undefined"){
            if (confirm('Confirm User Delete')) {
                ...
                $.ajax({
                url:baseUrl+'common/delete_from_table/tbl_users/user_id/'+data_row.user_id,
                type: 'POST', 
                data:{[csrfName]: csrfHash},
                dataType: 'json',
                success:function(data){ 
    ...
    $('#tbl_userdata tbody').off( 'click', '.editUser' ).on( 'click', '.editUser', function (e) {
        e.preventDefault();
        ...
        if (typeof(data_row) != "undefined"){        
            ...
            $.ajax({
            url:baseUrl+'common/get_selected_row/tbl_users/user_id/'+data_row.user_id,
            type: 'POST',
            dataType: 'json', 
            data:{[csrfName]: csrfHash},
            dataType: 'json',
            success:function(data,json){
                $('.txt_csrfname').val(data.token); 
                document.getElementById("register_firstname").value = data.response[0].firstname;
                document.getElementById("register_lastname").value = data.response[0].lastname;
                document.getElementById("register_username").value = data.response[0].username;
                document.getElementById("register_email").value = data.response[0].email;
                document.getElementById("register_password").value = data.response[0].password;
                document.getElementById("register_repassword").value = data.response[0].password;
                $('#user_status').val(data.response[0].user_status);
                document.getElementById("user_id").value = data.response[0].user_id;
            },
            complete: function(){
            },
            error:function (xhr, ajaxOptions, thrownError){alert(thrownError);}
...
function loadAnyListingsData(){
    ...
    "ajax": {
    "type" : "GET",
    "url" : baseUrl+'common/get_table_data_with_name/tbl_purchases',
    "data": {[csrfName]: csrfHash},
    "success": function (data,json) {
        $('.txt_csrfname').val(data.token);  
...
function getAnyListingsData(data){
    var csrfName    = $('.txt_csrfname').attr('name');
    var csrfHash    = $('.txt_csrfname').val();
    $.getJSON(baseUrl+'common/get_table_data_with_name/tbl_purchases',{[csrfName]: csrfHash},function(userData){
    $('.txt_csrfname').val(data.token);  
    var table = $('#tbl_anylistings').DataTable({
    destroy: true,
    data : data,
...
/* Load Withdrawals
/*--------------------------------------------------*/
function loadWithdrawalsDataRef(status){
    var csrfName    = $('.txt_csrfname').attr('name');
    var csrfHash    = $('.txt_csrfname').val();
    var table       = $('#tbl_withdrawals_ref').DataTable( {
    destroy:true,
    "ajax": {
    "type" : "GET",
    "url" : baseUrl+'refferal/withdrawals_data_ref/'+status,
    "data": {[csrfName]: csrfHash},
    "success": function (data,json) {
        $('.txt_csrfname').val(data.token); 
        getWithdrawalsDataRef(data.response);
    }}
    });
}
...
/*  Withdrawals Data Controls
/*--------------------------------------------------*/
function getWithdrawalsDataRef(data){
    var table = $('#tbl_withdrawals_ref').DataTable( {
    destroy: true,
    data : data,
    "columns": [
    ...
    $('#tbl_withdrawals_ref tbody').off( 'click', '.viewUser' ).on( 'click', '.viewUser', function (e) {
        e.preventDefault();
        var data_row = table.row($(this).closest('tr')).data();
        if (typeof(data_row) != "undefined"){
            $('#paypal_email').val(data_row.paypal);
    ...
    $('#tbl_withdrawals_ref tbody').off( 'click', '.inactiveClick' ).on( 'click', '.inactiveClick', function (e) {
        e.preventDefault();
        ...
        if (typeof(data_row) != "undefined"){
            if (confirm('Confirm Withdrawal Request Rejection ?')) {
                ...
                $('#loaderapp').show();             
                $.ajax({
                url:baseUrl+'refferal/update_selected__withdrawal/tbl_withdrawals_ref/status/3/id/'+data_row.id,
                type: 'POST',
                data:{[csrfName]: csrfHash},
                dataType: 'json', 
                success:function(data){
    ...
    $('#tbl_withdrawals_ref tbody').off( 'click', '.activeClick' ).on( 'click', '.activeClick', function (e) {
        e.preventDefault();
        ...
        if (typeof(data_row) != "undefined"){
            if (confirm('Confirm Withdrawal Request Approval ?')) {
                ...
                $('#loaderapp').show();               
                $.ajax({
                url:baseUrl+'refferal/update_selected__withdrawal/tbl_withdrawals_ref/status/2/id/'+data_row.id,
                type: 'POST', 
                data:{[csrfName]: csrfHash},
                dataType: 'json',
                success:function(data){ 
