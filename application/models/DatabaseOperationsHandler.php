...
class DatabaseOperationsHandler extends CI_Model
{
    ...
    /* Return Row Data Array*/
    public function _get_listing_data_name($table_name){
        $this->db->select('*, tbl_listings.website_BusinessName as website_BusinessName');
    ...
    public function _get_auction_data($condition,$limit,$ending=false,$count=false,$start,$void="",$sold=false){
        ...
        $this->db->group_start();
        $this->db->where($condition);
        $this->db->where_not_in('tbl_listings.status',array(0,6,2));
        if($sold){
